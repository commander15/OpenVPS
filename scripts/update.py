...
from utils.docker import run_docker_compose_pull

GIT_BRANCH = "main"

def update(exitOnError: bool = False):
    # Git Fetch
    run_command('git', [ 'fetch', 'origin', GIT_BRANCH ], printOutput=True, exitOnError=exitOnError)

    # Git Reset
    run_command('git', ['reset', '--hard', f'origin/{GIT_BRANCH}' ], printOutput=True, exitOnError=exitOnError)

    # Docker pull
    run_docker_compose_pull(exitOnError=exitOnError)
